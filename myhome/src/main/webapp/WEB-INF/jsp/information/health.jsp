<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
    <%@ include file="/WEB-INF/include/header.jspf" %>
			                    ...
			                    <c:forEach items="${list}" var="item">
			                        <div class="accordion-item">
			                            <div class="accordion-header" onclick="goContents('${item.contents_id}')">
			                                 <i class="fa fa-circle"></i> <a href='/information/health/info/${item.contents_id}.dr'>${item.title}</a>
			                            </div>
			                        </div>
			                    </c:forEach>
			                    </div>
		                        ...
		                        <div class="product-sidebar">
			                        <div class="block block-list block-viewed">
				                        <div class="block-title text-align-left padding-left20">
	                                        <strong><span>최근 본 건강정보</span></strong>
	                                    </div>
				                        <ul class="product-list-subset">
				                            <c:forEach items="${recentViewedList}" var="item">
				                                <li>
				                                    <h5 style="font-size: 16px;">
				                                        <a href='/information/${item.code}/info/${item.contents_id}.dr'>${item.title}</a>
				                                    </h5>
				                                </li>
				                            </c:forEach>
				                        </ul>
        ...
        function goContents(contents_id) {
            ...
            comSubmit.setUrl("<c:url value='/information/health/info/"+contents_id+".dr' />");
