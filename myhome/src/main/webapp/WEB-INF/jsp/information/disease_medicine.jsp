			            ...
			            <div class="product-sidebar">
			                <div class="block block-list block-viewed">
							    <div class="block-title">
							        <strong><span>필수 추천 생약제</span></strong>
							    </div>
							    <div class="block-content">
							        <ol id="recently-viewed-items">
							        <c:forEach items="${list1}" var="item1">
							           <li class="item odd">
							               <a href="/product/${item1.product_id}.dr" class="product-image" style="width:70px">
							                    <img class="gor-lazy" src="/image/${item1.image}" data-src="/image/${item1.image}" style="width:auto;height:90px;display: block;margin-left: auto;margin-right: auto;" alt="${item1.name}" title="${item1.name}">
							                </a>
							                <p class="product-name"><a href="/product/${item1.product_id}.dr">${item1.name}</a></p>
							            </li>
							        </c:forEach>
							        </ol>
							    </div>
							</div>
	                       <div class="block block-list block-viewed" style="padding-top:30px;">
                                <div class="block-title">
                                    <strong><span>추가 추천 생약제</span></strong>
                                </div>
                                <div class="block-content">
                                    <ol id="recently-viewed-items">
                                    <c:forEach items="${list2}" var="item2">
                                        <li class="item odd">
                                            <a href="/product/${item2.product_id}.dr" class="product-image" style="width:70px">
                                                <img class="gor-lazy" src="/image/${item2.image}" data-src="/image/${item2.image}" style="width:auto;height:100px;display: block;margin-left: auto;margin-right: auto;" alt="${item2.name}" title="${item2.name}">
                                            </a>
                                            <p class="product-name"><a href="/product/${item2.product_id}.dr">${item2.name}</a></p>
                                        </li>
                                    </c:forEach>
                                    </ol>
                                </div>
